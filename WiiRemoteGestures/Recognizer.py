...
class Recognizer:
    def __init__(self, feature_count, class_count, max_length):
        ...
        with tf.variable_scope('variables'):     
            # projection matrix
            self.weight = tf.Variable(tf.truncated_normal([num_mem_cells, class_count], stddev = 0.2))
            self.bias = tf.Variable(tf.constant(0.3, shape=[class_count]))
            self.conv_filter = tf.Variable(tf.truncated_normal([conv_height, conv_width, conv_in_channels, conv_out_channels], stddev=0.2))
            self.conv_biases = tf.Variable(tf.constant(0.3, shape=[conv_out_channels]))
            
        # recurent cells
        cells = [
            tf.contrib.rnn.GRUCell(num_mem_cells)
        ]
        cell = tf.contrib.rnn.MultiRNNCell(cells * 2, state_is_tuple=True)
        ...
        # INPUT DATA
            ...
            with tf.variable_scope('operations'):
                ...
                new_length = max_length - conv_width + 1
                cell_in = tf.reshape(conv, (-1, new_length, conv_out_channels))
                ...
                prediction = tf.reshape(prediction, (tf.shape(self.examples)[0], new_length, class_count), name="predictions")
                
                labels = tf.tile(tf.reshape(self.labels, [-1, 1]), [1, new_length]) #tf.slice(self.labels, [0, 0], [tf.shape(self.examples)[0], new_length])
                ...
                self.prediction = tf.reduce_sum(tf.nn.softmax(prediction) * tf.reshape(tf.to_float(mask), [-1, new_length, 1]), 1)
    ...
    def dropout_activation(self, input):
        return tf.nn.dropout(tf.tanh(input), self.keep_prob)
